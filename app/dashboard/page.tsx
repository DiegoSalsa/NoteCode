...
function SkeletonLoader({ rows = 3 }: { rows?: number }) {
  return (
    <div className="animate-pulse rounded-lg border border-white/10 bg-neutral-900">
      {Array.from({ length: rows }).map((_, index) => (
        <div
          key={index}
          className={`px-5 py-4 ${index !== rows - 1 ? "border-b border-white/5" : ""}`}
        >
    ...
    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
      {summaryItems.map((item) => {
        ...
        return (
          <div
            key={item.label}
            className="flex items-center gap-4 rounded-lg border border-white/10 bg-neutral-900 px-5 py-4 transition-colors hover:bg-neutral-900/80"
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[13px] font-medium text-neutral-100">{item.label}</p>
              <p className="mt-0.5 text-[12px] text-neutral-500">{item.subtitle}</p>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-white/10 bg-neutral-900">
      {projects.map((project, index) => (
        <div
          key={project.id}
          className={`flex items-start gap-3 px-4 py-4 transition-colors hover:bg-white/[0.03] sm:items-center sm:gap-4 sm:px-5 sm:py-3.5 ${
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <div className="min-w-0">
              <ProjectPrefetchLink
                ...
                className="truncate text-[14px] font-medium text-neutral-200 transition-colors hover:text-neutral-100 hover:underline"
              >
                {project.name}
              </ProjectPrefetchLink>
              <p className="mt-0.5 text-[12px] text-neutral-500">{project.client.name}</p>
            </div>
      ...
      {projects.length === 0 && (
        <div className="px-5 py-10 text-center text-[13px] text-neutral-500">
          No hay proyectos aun. Crea tu primer proyecto en la seccion Proyectos.
        </div>
      )}
    </div>
  );
...
export default async function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10 px-4 py-6 sm:px-6 sm:py-8 lg:space-y-14 lg:px-8 lg:py-10">
      ...
      <section>
        <Suspense fallback={<SkeletonLoader rows={3} />}>
          <SummaryCards />
        </Suspense>
      </section>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-[13px] text-neutral-500">
              Ultima actividad en los proyectos de la agencia.
            </p>
          </div>
          <Link
            href="/proyectos"
            className="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[13px] font-medium text-neutral-400 transition-colors hover:bg-white/5 hover:text-neutral-200"
          >
            <span>Ver todos</span>
